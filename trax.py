...
UART.setup("UART5")
UART.setup("UART2")
# ser = serial.Serial(port = "/dev/ttyO5", baudrate = 38400, timeout = 1)
# ser.close()
# ser.open()
    ...
    def start(self):
        ...
        while 1:
            ...
            if ord(f[0]) == 5:   # kGetDataResp has frameID = 5
                # first byte in the bytestring is ID count (uint8), followed by individual ID (uint8) and value (ID-specific format)
                data = struct.unpack(">BBBfBBBfBfBfBfBfBfB?B?BfBfBfBfBfBfBffff", f) #according to order of kSetDataComponents_all

                trax.heading.set(data[3])
                trax.heading_status.set(data[5]) # heading uncertainty
                trax.pitch.set(data[7])
                trax.roll.set(data[9])

                # rad/s -> deg/s
                trax.heading_rate.set(math.degrees(data[11]))
                trax.pitch_rate.set(math.degrees(data[13]))
                trax.roll_rate.set(math.degrees(data[15]))

                trax.temperature.set(data[17]) # in Celsius
                trax.distortion.set(data[19])
                trax.calibrated.set(data[21]) # user calibrated?

                # G -> m/s
                trax.accelx.set(data[23] * 9.8)
                trax.accely.set(data[25] * 9.8)
                trax.accelz.set(data[27] * 9.8)

                # in micro Tesla
                trax.magx.set(data[29])
                trax.magy.set(data[31])
                trax.magz.set(data[33])
